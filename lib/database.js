...
require("pg/lib/defaults").poolIdleTimeout = 3153600000000;
...
function getConnection(config, callback) {
	...
	var stack = exports.DEBUG ? new Error().stack : null;
	...
	pg.connect(config, function(err, con, done) {
		...
		if(exports.DEBUG) {
			...
			con.done = function() {
				var idx = _dbConnections.indexOf(con);
				if(idx == -1)
					console.warn("Connection.done() called twice?", con.stack);
				else
					_dbConnections = _dbConnections.slice(0, idx).concat(_dbConnections.slice(idx+1));
...
exports._getConnections = _getConnections;