...
function _shouldListKey(keyring, keyId, callback) {
	async.waterfall([
		function(next) {
			_exists(keyring._con, "keys_identities", { key: keyId }, null, next);
		},
		function(exists, next) {
			if(exists)
				_exists(keyring._con, "keys_attributes", { key: keyId }, null, next);
			else
				next(null, false);
		},
		function(exists, next) {
			if(exists)
				_exists(keyring._con, "keys_subkeys", { parentkey: keyId }, null, next);
			else
				next(null, false);
		},
		function(exists, next) {
			if(exists)
				_exists(keyring._con, "keys_signatures", { key: keyId }, null, next);
			else
				next(null, false);
		}
	], callback);
