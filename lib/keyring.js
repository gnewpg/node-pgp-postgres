...
var tableFields = { };
tableFields.keys = [ "id", "fingerprint", "binary", "date", "expires", "revoked", "security", "primary_identity" ];
tableFields.keys_subkeys = [ "parentkey", "id", "fingerprint", "binary", "security", "date", "security" ];
tableFields.keys_subkeys_selfsigned = [ "parentkey", "id", "fingerprint", "binary", "security", "date", "expires", "revoked", "security" ];
tableFields.keys_identities = [ "key", "id", "name", "email", "nameTrust", "emailTrust" ];
tableFields.keys_identities_selfsigned = [ "key", "id", "name", "email", "expires", "revoked", "security", "nameTrust", "emailTrust" ];
tableFields.keys_attributes = [ "key", "id", "binary", "trust" ];
tableFields.keys_attributes_selfsigned = [ "key", "id", "binary", "expires", "revoked", "security", "trust" ];
tableFields.keys_signatures = [ "key", "id", "issuer", "date", "binary", "verified", "sigtype", "expires", "revoked", "security", "trustSignature" ];
tableFields.keys_identities_signatures = [ "key", "identity", "id", "issuer", "date", "binary", "verified", "sigtype", "expires", "revoked", "security", "trustSignature" ];
tableFields.keys_attributes_signatures = [ "key", "attribute", "id", "issuer", "date", "binary", "verified", "sigtype", "expires", "revoked", "security", "trustSignature" ];
tableFields.keys_signatures_all = [ "id", "key", "issuer", "date", "binary", "verified", "sigtype", "expires", "revoked", "type", "objectId", "trustSignature" ];
...
pgp.utils.extend(_KeyringPostgres.prototype, {
	...
	subkeyExists : function(keyId, id, callback, _isTryingToAdd) {
		if(_isTryingToAdd)
			return this.keyExists(id, callback, _isTryingToAdd);
	...
	getSelfSignedSubkeys : function(keyId, filter, fields) {
		return _getEntries(this._con, "keys_subkeys_selfsigned", pgp.utils.extend({ parentkey: keyId }, filter), fields, pgp.packetContent.getPublicSubkeyPacketInfo);
	},

	getSelfSignedSubkey : function(keyId, id, callback, fields) {
		_getEntry(this._con, "keys_subkeys_selfsigned", { parentkey: keyId, id: id }, fields, pgp.packetContent.getPublicSubkeyPacketInfo, callback);
	},
	...
	getKeySignatureListByIssuer : function(issuerId, filter) {
		...
		return _getEntries(this._con, "keys_signatures", filter, [ "key", "id" ], pgp.packetContent.getSignaturePacketInfo).map(function(signatureRecord, callback) {
	...
	getSubkeySignature : function(keyId, subkeyId, id, callback, fields) {
		_getEntry(this._con, "keys_signatures", { key: subkeyId, issuer: keyId, id: id, sigtype: SUBKEY_SIGTYPES }, fields, pgp.packetContent.getSignaturePacketInfo, callback);
	},
	...
	getIdentitySignatureListByIssuer : function(issuerId, filter) {
		return _getEntries(this._con, "keys_identities_signatures", pgp.utils.extend({ issuer: issuerId }, filter), [ "key", "identity", "id" ], pgp.packetContent.getSignaturePacketInfo).map(function(signatureRecord, callback) {
	...
	getAttributeSignatureListByIssuer : function(issuerId, filter) {
		return _getEntries(this._con, "keys_attributes_signatures", pgp.utils.extend({ issuer: issuerId }, filter), [ "key", "attribute", "id" ], pgp.packetContent.getSignaturePacketInfo).map(function(signatureRecord, callback) {
	...
	getSignatureById : function(signatureId, callback, fields) {
		if(fields)
		{
			if(fields.indexOf("type") == -1)
				fields.push("type");
			if(fields.indexOf("objectId") == -1)
				fields.push("objectId");
			if(fields.indexOf("key") == -1)
				fields.push("key");
		}

		_getEntry(this._con, "keys_signatures_all", { id: signatureId, verified: true }, fields, pgp.packetContent.getSignaturePacketInfo, function(err, signatureInfo) {
			if(err || signatureInfo == null)
				return callback(err, signatureInfo);

			if(signatureInfo.type == "identity")
				signatureInfo.identity = signatureInfo.objectId;
			else if(signatureInfo.type == "attribute")
				signatureInfo.attribute = signatureInfo.objectId;
			else if([ pgp.consts.SIG.SUBKEY, pgp.consts.SIG.SUBKEY_REVOK ].indexOf(signatureInfo.sigtype) != -1)
			{
				signatureInfo.subkey = signatureInfo.key;
				signatureInfo.key = signatureInfo.issuer;
			}

			delete signatureInfo.type;
			delete signatureInfo.objectId;

			callback(null, signatureInfo);
		});
	},

	getAllSignatures : function(keyId, filter, fields) {
		return _getEntries(this._con, "keys_signatures_all", pgp.utils.extend({ key: keyId }, filter), fields, pgp.packetContent.getSignaturePacketInfo);
	},
	...
	searchIdentities : function(searchString) {
		var query = 'SELECT "keys"."id" AS "keyId", "keys"."expires" AS "keyExpires", "keys"."revoked" AS "keyRevoked", "found"."id" AS "id", "found"."name" AS "name", "found"."email" AS "email", "found"."expires" AS "expires", "found"."revoked" AS "revoked" FROM "keys_identities_selfsigned" AS "found", "keys" WHERE "found"."key" = "keys"."id" AND "found"."id" ILIKE \'%\' || $1 || \'%\'';
		return db.fifoQuery(this._con, query, [ searchString ]).map(function(record, callback) {
	...
	_getOwnerTrustInfo : function(keyId, filter, fields) {
		return db.getEntries(this._con, "ownertrust", fields, pgp.utils.extend({ key: keyId }, filter));
	},

	_addOwnerTrustInfo : function(keyId, trustInfo, callback) {
		_startTransaction(this._con, function(err) {
			if(err)
				return callback(err);

			db.insert(this._con, "ownertrust", pgp.utils.extend({ key: keyId }, trustInfo), callback);
		}.bind(this));
	},
	...
	_removeOwnerTrustBySignature : function(signatureId) {
		...
		_startTransaction(this._con, function(err) {
			...
			ret._add(db.fifoQuery(this._con, 'DELETE FROM "ownertrust" WHERE $1 = ANY("signaturePath") RETURNING *', [ signatureId ]));
		}.bind(this));
	...
	_removeKeyTrust : function(keyId) {
		...
		_startTransaction(this._con, function(err) {
			...
			ret._add(db.fifoQuery(this._con, 'DELETE FROM "ownertrust" WHERE ( "key" = $1 AND array_length("keyPath", 1) IS NULL ) OR "keyPath"[1] = $1 RETURNING *', [ keyId ]));
			ret._end();
		}.bind(this));
...
function _getEntries(con, table, filter, fields, getPacketInfo) {
	...
	if((table == "keys_identities" || table == "keys_identities_selfsigned") && fetchFields["binary"])
	{
		fetchFields["id"] = true;
	...
	// Nasty hack to live without the "binary" column for identities
	if(table == "keys_identities" || table == "keys_identities_selfsigned")
	{
		ret = ret.map(function(identityInfo, cb) {
	...
	if(shouldGetPacketInfo)
	{
		ret = ret.map(function(item, callback) {
			getPacketInfo(item.binary, function(err, packetInfo) {
				if(err)
					return callback(err);
				callback(null, pgp.utils.extend(packetInfo, item));
			});
		});
	}

	ret = pgp.Keyring._filter(ret, filterSplit.js).map(function(item, callback) {
		callback(null, pgp.Keyring._strip(item, fields));
	});
...
function _getList(con, table, idField, filter, getPacketInfo) {
	...
	return entries.map(function(item, callback) {
...
function _isSubkey(con, keyId, callback) {
	db.entryExists(con, "keys_signatures", { key: keyId, sigtype: SUBKEY_SIGTYPES }, callback);
}
	...
	result.forEachSeries(function(it, next) {
		async.auto({
			...
			addParentKeys: [ "addKey", function(next, results) {
				_getEntries(keyring._con, "keys_subkeys_selfsigned", { id: it.id }, [ "parentkey", "expires", "revoked" ], null).forEachSeries(function(it2, next) {
