...
pgp.utils.extend(_KeyringPostgres.prototype, {
	...
	_removeOwnerTrustBySignature : function(signatureId) {
		return db.fifoQuery(this._con, 'DELETE FROM "ownertrust" WHERE $1 = ANY("signaturePath") RETURNING *', [ signatureId ]);
	},

	_removeKeyTrust : function(keyId) {
		var ret = new pgp.Fifo();
		db.query(this._con, 'DELETE FROM "ownertrust" WHERE "key" = $1 AND array_length("signaturePath", 1) IS NULL', [ keyId ], function(err) {
			if(err)
				return ret._end(err);

			ret._add(db.fifoQuery(this._con, 'DELETE FROM "ownertrust" WHERE "keyPath"[1] = $1 RETURNING *', [ keyId ]));
			ret._end();
		}.bind(this));
