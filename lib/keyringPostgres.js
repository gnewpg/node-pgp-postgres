...
// TODO: keyring._checkSelfSignatures requires key.expiresOrig

var tableFields = { };
tableFields.keys = [ "id", "fingerprint", "binary", "date", "expires", "revoked", "security" ];
tableFields.keys_subkeys = [ "id", "binary", "security", "date" ];
...
function getKeyring(dbConfig, callback) {
	db.getConnection(dbConfig, function(err, con) {
		if(err)
			callback(err);
		else
			callback(null, new _KeyringPostgres(con));
	});
}
...
pgp.utils.extend(_KeyringPostgres.prototype, {
	...
	keyExists : function(id, callback) {
		return _exists(this._con, "keys", { id: id }, pgp.packetContent.getPublicKeyPacketInfo, callback);
	},

	getKey : function(id, callback, fields) {
		return _getEntry(this._con, "keys", { id: id }, fields, pgp.packetContent.getPublicKeyPacketInfo, callback);
	},
	...
	subkeyExists : function(keyId, id, callback) {
		return _exists(this._con, "keys_subkeys", { parentkey: keyId, id: id }, pgp.packetContent.getPublicSubkeyPacketInfo, callback);
	},

	getSubkey : function(keyId, id, callback, fields) {
		return _getEntry(this._con, "keys_subkeys", { parentkey: keyId, id: id }, fields, pgp.packetContent.getPublicSubkeyPacketInfo, callback);
	},
	...
	identityExists : function(keyId, id, callback) {
		return _exists(this._con, "keys_identities", { key: keyId, id: id }, pgp.packetContent.getIdentityPacketInfo, callback);
	},

	getIdentity : function(keyId, id, callback, fields) {
		return _getEntry(this._con, "keys_identities", { key: keyId, id: id }, fields, pgp.packetContent.getIdentityPacketInfo, callback);
	},
	...
	attributeExists : function(keyId, id, callback) {
		return _exists(this._con, "keys_attributes", { key: keyId, id: id }, pgp.packetContent.getAttributePacketInfo, callback);
	},

	getAttribute : function(keyId, id, callback, fields) {
		return _getEntry(this._con, "keys_attributes", { key: keyId, id: id }, fields, pgp.packetContent.getAttributePacketInfo, callback);
	},
	...
	getKeySignatureListByIssuer : function(issuerId, filter) {
		...
		return Fifo.map(_getEntries(this._con, "keys_signatures", filter, [ "key", "id" ], pgp.packetContent.getSignaturePacketInfo), function(signatureRecord, callback) {
			callback(null, { key: signatureRecord.key, signature: signatureRecord.id });
		});
	},

	keySignatureExists : function(keyId, id, callback) {
		return _exists(this._con, "keys_signatures", { key: keyId, id: id, sigtype: KEY_SIGTYPES }, pgp.packetContent.getSignaturePacketInfo, callback);
	},

	getKeySignature : function(keyId, id, callback, fields) {
		return _getEntry(this._con, "keys_signatures", { key: keyId, id: id, sigtype: KEY_SIGTYPES }, fields, pgp.packetContent.getSignaturePacketInfo, callback);
	},
	...
	getSubkeySignatureListByIssuer : function(issuerId, filter, fields) {
		filter = pgp.utils.extend({ issuer: issuerId, sigtype: SUBKEY_SIGTYPES }, filter);
		return Fifo.map(_getEntries(this._con, "keys_signatures", filter, [ "key", "id" ], pgp.packetContent.getSignaturePacketInfo), function(signatureRecord, callback) {
			callback(null, { key: issuerId, subkey: signatureRecord.key, signature: signatureRecord.id });
		});
	},

	subkeySignatureExists : function(keyId, subkeyId, id, callback) {
		return _exists(this._con, "keys_signatures", { key: subkeyId, issuer: keyId, id: id, sigtype: SUBKEY_SIGTYPES }, pgp.packetContent.getSignaturePacketInfo, callback);
	},

	getSubkeySignature : function(keyId, subkeyId, id, callback, fields) {
		return _getEntry(this._con, "keys_signatures", { key: keyId, id: id, sigtype: SUBKEY_SIGTYPES }, fields, pgp.packetContent.getSignaturePacketInfo, callback);
	},
	...
	getIdentitySignatureListByIssuer : function(issuerId, filter, fields) {
		return Fifo.map(_getEntries(this._con, "keys_identities_signatures", pgp.utils.extend({ issuer: issuerId }, filter), [ "key", "identity", "id" ], pgp.packetContent.getSignaturePacketInfo), function(signatureRecord, callback) {
			callback(null, { key: signatureRecord.key, identity: signatureRecord.identity, signature: signatureRecord.id });
		});
	},

	identitySignatureExists : function(keyId, identityId, id, callback) {
		return _exists(this._con, "keys_identities_signatures", { key: keyId, identity: identityId, id: id }, pgp.packetContent.getSignaturePacketInfo, callback);
	},

	getIdentitySignature : function(keyId, identityId, id, callback, fields) {
		return _getEntry(this._con, "keys_identities_signatures", { key: keyId, identity: identityId, id: id }, fields, pgp.packetContent.getSignaturePacketInfo, callback);
	},
	...
	getAttributeSignatureListByIssuer : function(issuerId, filter, fields) {
		return Fifo.map(_getEntries(this._con, "keys_attributes_signatures", pgp.utils.extend({ issuer: issuerId }, filter), [ "key", "attribute", "id" ], pgp.packetContent.getSignaturePacketInfo), function(signatureRecord, callback) {
			callback(null, { key: signatureRecord.key, attribute: signatureRecord.attribute, signature: signatureRecord.id });
		});
	},

	attributeSignatureExists : function(keyId, attributeId, id, callback) {
		return _exists(this._con, "keys_attributes_signatures", { key: keyId, attribute: attributeId, id: id }, pgp.packetContent.getSignaturePacketInfo, callback);
	},

	getAttributeSignature : function(keyId, attributeId, id, callback, fields) {
		return _getEntry(this._con, "keys_attributes_signatures", { key: keyId, attribute: attributeId, id: id }, fields, pgp.packetContent.getSignaturePacketInfo, callback);
	},
	...
	_updateAttributeSignature : function(keyId, attributeId, signatureId, fields, callback) {
		_updateEntry(this._con, "keys_identities_signatures", fields, { key: keyId, attribute: attributeId, id: signatureId }, [ "key", "attribute", "id" ], pgp.packetContent.getSignaturePacketInfo, callback);
	},
...
function _getEntries(con, table, filter, fields, getPacketInfo) {
	...
	var filterSplit = _splitFilter(filter);
	...
	if(shouldGetPacketInfo)
	{
		ret = Keyring._strip(pgp.Fifo.map(ret, function(item, callback) {
			getPacketInfo(item.binary, function(err, packetInfo) {
				if(err)
					return callback(err);
				callback(null, utils.extend(packetInfo, item));
			});
		}), fields);
	}
	ret = Keyring._filter(ret, filterSplit.js);

	return ret;
}

function _getList(con, table, idField, filter, getPacketInfo) {
	var entries = _getEntries(con, table, [ idField ], filter, getPacketInfo);
	...
	async.waterfall([
		...
		function(next) {
			...
			if(exists)
				...
				db.entryExists(con, "keys_signatures", { issuer: keyId, sigtype: SUBKEY_SIGTYPES, key: Keyring.Filter.Not(exceptSubkeyId) }, next);
		},
